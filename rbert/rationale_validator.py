...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, encodings, labels, first_token_indices):
        ...
        self.first_token_indices = first_token_indices # index of the first token in the original tokenization
...
def encode_tags(rationale, encodings, unk_rationale_id):
    ...
    for doc_labels, doc_offset in zip(rationale, encodings.offset_mapping):
        ...
        assert(sum(first_token_indices) == len(doc_labels)), f"fst tkn {sum(first_token_indices)}, doc_labels {len(doc_labels)}"
...
def preprocess_data(args, data_json):
    """
    ...
    labels, first_token_indices = encode_tags(rationale, encodings, args.unk_rationale_id)
...
def view_rationale(tokens, rationale):
    assert len(tokens) == len(rationale), f"tokens: {len(tokens)}, rationale:{len(rationale)}"
    pretty_text = ""
    for i, token in enumerate(tokens):
        if rationale[i] == 1:
            pretty_text += '['+token+']' + " "
        else: pretty_text += token + " "
    return pretty_text

def print_human_vs_model(test_json, pred_rationale):
    assert len(test_json) == len(pred_rationale), "test rationale size and test dataset size are different"
...
def print_score(args, total, correct):
    ...
    for c in list_of_classes:
        print(f"{c}\t{correct[c]}\t{max(total[c], 1)}\t{correct[c]/max(total[c], 1)*100}")
...
def insert_symbol(x, symbol, start, end, other_start, other_end):
    ...
    x["token"] = x["token"][:start] + [symbol] + x['token'][start:end+1] + [symbol] + x['token'][end+1:]
    x["rationale"] = x["rationale"][:start] + [True] + x['rationale'][start:end+1] + [True] + x['rationale'][end+1:]
    ...
    assert other_entity == new_other_entity, f"{other_entity}==={new_other_entity}, {x['token']}"
...
def delete_symbol(x, symbol, start, end, other_start, other_end):
    ...
    x["token"] = x["token"][:start-1] + x['token'][start:end+1] + x['token'][end+2:]
    x["rationale"] = x["rationale"][:start-1] + x['rationale'][start:end+1] + x['rationale'][end+2:]
...
def main():
    ...
    parser.add_argument('--data_dir', help="path to data directory", default="./data/ace2005/tacred/")
    parser.add_argument('--ratdataset', help="rationale file name", default="rationale.json")
    ...
    parser.add_argument('--unk_rationale_id', help="rationale id to ignore a token", default=-100, type=int)
    ...
    train_loader = DataLoader(preprocess_data(args, train_json), batch_size=args.batch_size, shuffle=False)
    test_loader = DataLoader(preprocess_data(args, test_json), batch_size=args.batch_size, shuffle=False)
