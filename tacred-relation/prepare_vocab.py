...
def parse_args():
    ...
    parser.add_argument(
        "--wv_file", default="glove.840B.300d.txt", help="GloVe vector file."
    )
    parser.add_argument(
        "--wv_dim", type=int, default=300, help="GloVe vector dimension."
    )
    parser.add_argument(
        "--min_freq", type=int, default=0, help="If > 0, use min_freq as the cutoff."
    )
    parser.add_argument(
        "--lower", action="store_true", help="If specified, lowercase all words."
    )
...
def build_vocab(tokens, glove_vocab, min_freq):
    ...
    v = constant.VOCAB_PREFIX + list(constant.LABEL_TO_ID.keys()) + entity_masks() + v
