...
class RelationModel(object):
    ...
    def update(self, batch, biased_model_probs=None):
        ...
        if isinstance(biased_model_probs, torch.Tensor):
            ...
            loss -= torch.mean(
                torch.log(torch.einsum("ij,ij->i", probs, biased_model_probs))
            )
    ...
    def save(self, filename, epoch):
        params = {"model": self.model.state_dict(), "config": self.opt, "epoch": epoch}
        try:
...
class PositionAwareRNN(nn.Module):
    ...
    def __init__(self, opt, emb_matrix=None):
        ...
        if opt["pos_dim"] > 0:
            self.pos_emb = nn.Embedding(
                len(constant.POS_TO_ID), opt["pos_dim"], padding_idx=constant.PAD_ID
            )
        if opt["ner_dim"] > 0:
            self.ner_emb = nn.Embedding(
                len(constant.NER_TO_ID), opt["ner_dim"], padding_idx=constant.PAD_ID
            )
    ...
    def init_weights(self):
        if self.emb_matrix is None:
            self.emb.weight.data[1:, :].uniform_(
                -1.0, 1.0
            )  # keep padding dimension to be 0
        else:
        ...
        if self.topn <= 0:
            ...
            self.emb.weight.register_hook(
                lambda x: torch_utils.keep_partial_grad(x, self.topn)
            )
        else:
